...
curl -s \
  ...
  --data-binary "@mac/dist/datashare-$release.pkg" "$upload_url?name=Datashare.pkg&label=Datashare.pkg"

curl -s \
  -H "Authorization: token $GITHUB_TOKEN" \
  -H "Content-Type: application/vnd.microsoft.portable-executable" \
  --data-binary "@windows/dist/datashare-$release.exe" "$upload_url?name=installDatashare.exe&label=installDatashare.exe"
  
curl -s \
  -H "Authorization: token $GITHUB_TOKEN" \
  -H "Content-Type: application/x-sh" \
  --data-binary "@linux/dist/datashare-$release.sh" "$upload_url?name=datashare.sh&label=datashare.sh"
